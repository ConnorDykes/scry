...
exports.sendMessageNotification = functions.firestore
  ...
  .document("users/{userID}/offers/{offerID}")
  .onCreate(async (queryDocumentSnapshot, context) => {
    console.log("New Message Sent");

    // the actual message document that was created
    const offerDoc = queryDocumentSnapshot.data();
    const userID = context.params.userID;

    if (userID != offerDoc.recipientUserID) {
       // This value helps us increment the notifications field in the
    //user doc that we will use for app badge count
    console.log("Incrementing notification count.....");
    const FieldValue = require("firebase-admin").firestore.FieldValue;
    ...
    const message = {
      notification: {
        title: 'New Offer',
        body: offerDoc.offeringUserName + ' wants to trade their '+ offerDoc.offeredCards[0].name +  " for your " + offerDoc.availableCards[0].name,
      },
      token: fcmToken,
      apns: {
        payload: {
